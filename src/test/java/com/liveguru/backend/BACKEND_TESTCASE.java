...
public class BACKEND_TESTCASE extends AbstractTest {
	...
	public void TC_01_DeleteAccount() {
		...
		verifyEquals(BE_ManageCustomerPage.getDynamicRowOfTable(driver, CommonClass.email), CommonClass.email);
		...
		BE_ManageCustomerPage.selectDynamicDropdownInAction_BE(driver, "Delete", "customerGrid_massaction-select");
	...
	public void TC_02_DeleteAccount_DeleteReview() {
		...
		BE_ManageCustomerPage.selectDynamicDropdownInAction_BE(driver, "Update Status", "reviwGrid_massaction-select");

		BE_ManageCustomerPage.selectDynamicDropdownInAction_BE(driver, "Approved", "status");
		BE_ManageCustomerPage.clickToDynamicLinkButton(driver, "Submit");
		verifyTrue(
				BE_ManageCustomerPage.isApprovedStatusDisplayed(CommonClass.summaryReview, CommonClass.detailReview));
		BE_ManageCustomerPage.clickToDynamicRowInReviewTable(driver, CommonClass.summaryReview,
				CommonClass.detailReview);
		BE_ManageCustomerPage.selectDynamicDropdownInAction_BE(driver, "Delete", "reviwGrid_massaction-select");
		BE_ManageCustomerPage.clickToDynamicLinkButton(driver, "Submit");
		BE_ManageCustomerPage.clickOKToAcceptDelete();
	}
	
    @Test
    public void TC_03_InvoiceCanbePrinted() {
	...
	public void checkSearchTableByColumn(int column, String value) {
		...
		for (int i = 1; i <= rowTotal; i++) {
			WebElement elementCheck = driver
					.findElement(By.xpath("//table[@id='reviwGrid_table']//tbody//tr[" + i + "]//td[" + column + "]"));
...
//    }​
	public void TC_03_ControlTable() {
		List<WebElement> rows_table = driver.findElements(By.xpath("//table[@id='reviwGrid_table']//tbody//tr"));
		int colSize = rows_table.size();
		List<WebElement> column_table = driver
				.findElements(By.xpath("//table[@id='reviwGrid_table']//tbody//tr//td[4]"));
